const {
  ...
  deleteTask,
  mockDatabase,
} = require('../src/controllers/tasks.controller.js');
...
describe('Controlador Unitario de Tareas (tasks.controller.js)', () => {
  ...
  describe('updateTask', () => {
    ...
    test('Debe actualizar solo el estado "completed" a true y devolver 200', () => {
      const req = mockRequest({ completed: true }, { id: TASK_ID.toString() });
      const res = mockResponse();
    ...
    test('Debe devolver 404 si el ID no es un número válido (NaN después de parseInt)', () => {
      ...
      const req = mockRequest(
        { title: 'Test' },
        { id: 'abc' } // 'abc' -> parseInt() = NaN. El controlador ahora lo detecta primero.
      );
      const res = mockResponse();
  ...
  describe('deleteTask', () => {
    ...
    test('Debe eliminar la tarea y devolver 204 No Content', () => {
      const req = mockRequest({}, { id: TASK_ID.toString() });
      const res = mockResponse();
    ...
    test('Debe devolver 404 si el ID de eliminación no es un número válido (NaN)', () => {
      const req = mockRequest(
        {},
        { id: 'invalidID' } // 'invalidID' -> parseInt() = NaN.
      );
      const res = mockResponse();
