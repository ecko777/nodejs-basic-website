...
describe('Rutas de API de Tareas (/api/tasks)', () => {
  ...
  test('PUT /api/tasks/:id inexistente debe devolver status 404', async () => {
    // Capturamos la respuesta para poder verificar el cuerpo
    const response = await request(app)
    ...
    // Aserción explícita para satisfacer jest/expect-expect
  ...
  test('DELETE /api/tasks/:id debe eliminar la tarea y devolver status 204', async () => {
    ...
    await request(app).delete(`/api/tasks/${taskId}`).expect(204); // No Content

    // Aserción sobre la base de datos
    expect(mockDatabase.length).toBe(0);
  });
  ...
  test('DELETE /api/tasks/:id inexistente debe devolver status 404', async () => {
    ...
    // Aserción explícita para satisfacer jest/expect-expect y verificar el cuerpo
    expect(response.body).toHaveProperty('message', 'Tarea no encontrada');
  });
});
